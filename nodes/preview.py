...
class UniversalPreview:
    ...
    def execute(self, any):
        ...
        if isinstance(value, dict):
            if "waveform" in value and "sample_rate" in value:
                ...
                samples = w.shape[-1]
                duration = samples / sr
                text = f"[AUDIO]  batch: {b}  {sr}Hz  ch: {ch}  {duration:.2f}s"
            ...
            if "samples" in value:
                ...
                if isinstance(s, torch.Tensor) and s.dim() == 4:
                    b, c, h, w = s.shape
                    text = f"[LATENT]  batch: {b}  size: {w}×{h}  ch: {c}"
    ...
    def _format_value(self, value):
        ...
        if isinstance(value, (list, tuple)):
            ...
            if n > 10:
                preview += f"\n... ({n} items)"
            return f"[LIST<{first_type}>]  count: {n}\n{preview}"
        if isinstance(value, dict):
            try:
...
NODE_DISPLAY_NAME_MAPPINGS = {"Logic_UniversalPreview": "🖥️ Universal Preview"}
